...
*& Report
*&---------------------------------------------------------------------*
*& Auftrag/Incident: 4000000853/INC08949819 - fehlende Mahnstufen #3
*& Datum           : 23.03.2026
...
SELECTION-SCREEN BEGIN OF BLOCK b3 WITH FRAME TITLE TEXT-t03.
  ...
  PARAMETERS: p_spos RADIOBUTTON GROUP g1 DEFAULT 'X'.
  PARAMETERS: p_sjah RADIOBUTTON GROUP g1.
...
*********************************************************************
  ...
  so_blart[] = VALUE #( ( sign = 'I' option = 'EQ' low = 'DR' ) ).
  so_hf[]    = VALUE #( ( sign = 'I' option = 'EQ' low = 'H' ) ).
  so_posnr[] = VALUE #( ( sign = 'E' option = 'EQ' low = '0000000000' ) ).
  so_manst[] = VALUE #( ( sign = 'I' option = 'EQ' low = '0' ) ).
  so_migob[] = VALUE #( ( sign = 'I' option = 'EQ' low = 'SSTE' )
                        ( sign = 'I' option = 'EQ' low = 'SSTS' )
                        ( sign = 'I' option = 'EQ' low = 'SSTA' ) ).
*********************************************************************
START-OF-SELECTION.
*********************************************************************
...
* Daten von den
  SELECT a~*, b~stundungsende, b~einzelplan, b~kennzeichenstundung, b~adfschluessel, b~migrationsobjekt
...
*--------------------------------------------------------------------*
  IF p_test NE abap_true.
    ...
    LOOP AT gt_mig_data ASSIGNING <fs_mig_data> WHERE type IS INITIAL.
      ...
      IF <fs_mig_data>-blart NE 'DR'.
        <fs_mig_data>-type = 'W'.
        <fs_mig_data>-message = 'Bur Belegarten DR unterstützt'.
        CONTINUE.
      ENDIF.
