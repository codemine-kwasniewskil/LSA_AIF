 ...
 TYPES: BEGIN OF lty_data_tmp, " zur Laufzeit zu ermittelnde Daten, die nicht versorgt werden
  ...
  TRY.
      ...
      IF line_exists( ac_container[ element = lc_fipos ] ).                                          " Finanzposition
        ls_data_tmp-zfipos = ac_container[ element = lc_fipos ]-value.
      ENDIF.
      ...
      " Ermittle Genehmiger
      DATA(lt_plans) = lo_wf->get_approver( iv_wf_id            = lv_wf_id               " BAnf-Workflow
                                            iv_funk             = lv_wf_funk             " Operativer Einkäufer
      ...
      IF lt_plans IS INITIAL.
        ...
        " Ermittle OM Support
        lt_plans = lo_wf->get_approver( iv_wf_id            = lv_wf_id                                   " BAnf-Workflow
