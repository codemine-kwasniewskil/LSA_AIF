...
*&
*&---------------------------------------------------------------------*
REPORT /thkr/fi_elko_sgtxt_t999 LINE-SIZE 80.
...
* Beschreibung:
*
* Das Programm überträgt bei fehlerhaften Belegtexten im D/K des T999
* den Belegtext aus Position 1 des T999-Beleges in die Position 2
...
START-OF-SELECTION.
  ...
  FROM ( bseg  AS qs                "Quell-Segment (T999)
         ...
         ON  zs~belnr = qk~belnr
         AND zs~bukrs = qk~bukrs
         AND zs~gjahr = qk~gjahr )
       WHERE qs~buzei = 1           "Quelle ist die Zeile 1 des T999-Beleges
         ...
         AND zs~buzei = 2           "Ziel ist Zeile 2 des zahl. Beleges
