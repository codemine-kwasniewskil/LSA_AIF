class /THKR/CL_SALV_EXT_IF_RUN1 definition
  public
...
private section.
  ...
  constants C_REPORT_NAME type REPID value 'CL_SALV_EXT_IF_RUN1' ##NO_TEXT.
  data APPL type ref to /THKR/CL_EXT_IF_APPL .
  data T_AO1 type /THKR/T_DTO_DE_RUN_AO1 .
...
CLASS /THKR/CL_SALV_EXT_IF_RUN1 IMPLEMENTATION.
  ...
  METHOD fill_data.
    ...
    FIELD-SYMBOLS <selection> TYPE /thkr/s_de_run_ao1_selection.

    ASSIGN selection->* TO <selection>.

    appl->get_tdto_de_run_ao1(
      EXPORTING
        i_selection = <selection>
      IMPORTING
        et_dto      = t_ao1 ).

    GET REFERENCE OF t_ao1 INTO t_data_ref.
  ...
  METHOD handle_added_function.
    ...
    IF sy-subrc = 0.
      READ TABLE t_ao1 INDEX l_row ASSIGNING FIELD-SYMBOL(<line>).
