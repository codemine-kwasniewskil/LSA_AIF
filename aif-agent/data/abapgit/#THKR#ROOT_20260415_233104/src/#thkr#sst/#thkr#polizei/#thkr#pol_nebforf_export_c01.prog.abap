...
*    -----//-----
  PROTECTED SECTION.
    CLASS-DATA:
      ...
      mt_data          TYPE TABLE OF /thkr/sst_polizei_nebenford,
      mo_table         TYPE REF TO cl_salv_table,
      mv_r_blart       TYPE RANGE OF blart.
...
*    ENDLOOP.
  ...
  ENDMETHOD.
  METHOD get_data.
    "MG#MO#SG#SN#VK#VO#EL#HB#HO#AH#AN#AW#GK#GO#HA#
    mv_r_blart = VALUE #( sign = 'I' option = 'EQ' ( low = 'MG' ) ( low = 'MO' ) ( low = 'SG' )
                                                   ( low = 'SN' ) ( low = 'VK' ) ( low = 'VO' )
                                                   ( low = 'EL' ) ( low = 'HB' ) ( low = 'HO' )
                                                   ( low = 'AH' ) ( low = 'AN' ) ( low = 'AW' )
                                                   ( low = 'GK' ) ( low = 'GO' ) ( low = 'HA' ) ).
    SELECT FROM bsid_view
      FIELDS blart AS blart
            ,xblnr AS kassenzeichen
            ,SUM( wrbtr ) AS wrbtr
      WHERE blart IN @mv_r_blart
        AND gsber IN @s_gsber
      GROUP BY blart, xblnr
      INTO TABLE @mt_data.
  ENDMETHOD.
  METHOD file_save.
    ...
    LOOP AT lt_csv_output ASSIGNING FIELD-SYMBOL(<lv_csv_output>).
      SPLIT <lv_csv_output> AT ';' INTO TABLE DATA(lt_parts).
      DATA(lv_index_of_last_part) = lines( lt_parts ).
      IF lv_index_of_last_part GT 0.
        READ TABLE lt_parts ASSIGNING FIELD-SYMBOL(<lv_last_part>) INDEX lv_index_of_last_part.
        REPLACE ALL OCCURRENCES OF '.' IN <lv_last_part> WITH ''.
        CONCATENATE LINES OF lt_parts INTO <lv_csv_output> SEPARATED BY ';'.
      ENDIF.
    ENDLOOP.
