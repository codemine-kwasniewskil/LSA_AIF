FUNCTION /thkr/aif_fremdv_act_wrt_fi_ki .
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  DATA: lo_file            TYPE REF TO /thkr/cl_aif_rueck.
  DATA: lo_type            TYPE REF TO cl_abap_typedescr.
  DATA: ls_bic_header     TYPE /thkr/s_aif_bic_header.
  DATA: lv_eol            TYPE abap_cr_lf.


  FIELD-SYMBOLS: <lt_table> TYPE /thkr/t_sst_kidicap_bzg_income.
...
*"----------------------------------------------------------------------
  ...
  "KIDICAP -> contains individual target table for output file
  ASSIGN COMPONENT 'KIDICAP' OF STRUCTURE <ls_data> TO <lt_table>.
  ...
  IF <lt_table> IS ASSIGNED AND <lt_table> IS NOT INITIAL.
    ...
    "Fehlerhafte Buchungen nicht in die Datei schreiben
    DELETE <lt_table> WHERE status = 'E' OR status = 'A'.
    IF <lt_table> IS NOT INITIAL.
      "modify row.
      lo_file = NEW /thkr/cl_aif_rueck( ).
      lt_table = lo_file->modify_output_tab(
                   it_rueck_lines = <lt_table>                 " Tabelle von Strings
                 ).

      Lv_logical_filename = |/THKR/AIF_I_0007_001_IN|.
      "Andere Datei.

      "Dateiname muss für KIDICAP umgebaut werden.
      "KIDICAP braucht eindeutige Nummern im Dateiname ab der 8. Stelle des Dateinamens.
      "Origigal Dateiname: kidicap_abs_202501_1646.txt -> 1616 = eindeutige Kennung
      "BI041234.iab.4101 -> 4101 = Dienststelle = statisch Verarbeitung klappt nicht.
      "Daher wird der Dateiname umgebaut in IAB.4101.BI041234 -> Einzelpan und Generationsnummer am Ende der Datei.
      SPLIT <ls_common>-dateiname AT '.' INTO TABLE DATA(lt_nameparts).

      DATA(lv_filename) = |{ lt_nameparts[ 2 ] }.{ lt_nameparts[ 3 ] }.{ lt_nameparts[ 1 ] } |.
...
* Implement suitable error handling here
       ...
       iv_ifname = lv_ifname                 " Schnittstellenname
        ...
        IF  lv_file_count <> 0 AND lv_ovwr = abap_false.
          ...
          success = 'N'.
        ELSE.
          lv_eol =  COND #( WHEN lo_file->ms_pprop-cr_lf = 1 THEN cl_abap_char_utilities=>cr_lf(1)
                                                 WHEN lo_file->ms_pprop-cr_lf = 2 THEN cl_abap_char_utilities=>newline
                                                 WHEN lo_file->ms_pprop-cr_lf = 3 THEN cl_abap_char_utilities=>cr_lf
                                                 WHEN lo_file->ms_pprop-cr_lf IS INITIAL THEN cl_abap_char_utilities=>newline ).
          CALL METHOD lo_protokoll->write_file_from_string_table
            EXPORTING
              iv_output_filename = lv_output_filename
              it_rows            = lt_table[]
              iv_cp              = lo_file->ms_pprop-codepage
