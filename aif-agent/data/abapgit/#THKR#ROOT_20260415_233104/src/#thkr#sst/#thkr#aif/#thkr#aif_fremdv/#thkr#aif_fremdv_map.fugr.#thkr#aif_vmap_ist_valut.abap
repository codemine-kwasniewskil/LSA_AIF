FUNCTION /thkr/aif_vmap_ist_valut.
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  IF value_in IS NOT INITIAL.
    "Ermittlung der letzten Belegnummer
    ...
    IF sy-subrc <> 0.
      CLEAR value_out.
    ELSE.
      "Lesen des Zahldatums aus Positionsdaten
      DATA(lv_belnr) = lt_belnr[ 1 ]-belnr.
      SELECT SINGLE bs~valut
        FROM bseg AS bs
        INNER JOIN bkpf AS bk
        ON bk~belnr = bs~belnr
        WHERE bk~xblnr = @value_in
          AND bk~bukrs = @value_in2
          AND bk~gjahr = @value_in3
          AND bs~belnr = @lv_belnr
          AND bs~koart = 'S'    "Sachkonto
    INTO @DATA(lv_valut).
      IF sy-subrc = 0.
        value_out = |{ lv_valut DATE = COND #( WHEN value_in4 = 'USER' THEN cl_abap_format=>d_user
