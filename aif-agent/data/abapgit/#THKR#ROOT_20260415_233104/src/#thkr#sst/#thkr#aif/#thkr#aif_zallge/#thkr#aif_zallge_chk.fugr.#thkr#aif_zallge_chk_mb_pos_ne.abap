FUNCTION /THKR/AIF_ZALLGE_CHK_MB_POS_NE .
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     REFERENCE(DATA_STRUCT) TYPE  /THKR/S_DE_PSO_XML_FILE
*"     REFERENCE(DATA_LINE) TYPE  KBLK
...
error = /thkr/cl_pso_xml_processing=>get_instance( )->check_mb_bpos_does_not_exist(
                                                     EXPORTING
                                                       is_data_struct = data_struct
                                                       is_data_line   = data_line                 " Belegkopf: Manuelle Belegerfassung
                                                     CHANGING
