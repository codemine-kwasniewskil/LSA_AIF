...
* VALUE_OUT BLART
*"----------------------------------------------------------------------
FUNCTION /thkr/aif_zallge_chk_reproc .
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  DATA: lc_reproc TYPE REF TO /thkr/cl_aif_reproc.

  lc_reproc = NEW /thkr/cl_aif_reproc( ).

  DATA(lv_reproc_is_active) = lc_reproc->check_reproc_is_active( ).

  CASE lv_reproc_is_active.
    WHEN: abap_true.
      error = abap_false.
    WHEN: abap_false.
      error = abap_true.
