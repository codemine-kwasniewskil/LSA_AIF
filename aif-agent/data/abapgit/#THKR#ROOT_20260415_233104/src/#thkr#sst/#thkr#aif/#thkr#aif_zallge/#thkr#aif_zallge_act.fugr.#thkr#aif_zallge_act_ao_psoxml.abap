...
*"----------------------------------------------------------------------
FUNCTION /thkr/aif_zallge_act_ao_psoxml .
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  IF <ls_curr_line> IS ASSIGNED AND <ls_curr_line> IS NOT INITIAL.
    ...
    /thkr/cl_pso_xml_processing=>get_instance( )->process_ao(
      EXPORTING
        it_gp               = <ls_curr_line>-gp
        it_anordnungen      = <ls_data>-werte-anordnungen
      CHANGING
        ct_ao               = <ls_curr_line>-ao
        ct_return           = return_tab[]
        cv_success          = success                 " Erfolgskennzeichen
    ).

    "Verarbeitung Sollzugang, Sollabgang
    /thkr/cl_pso_xml_processing=>get_instance( )->process_ao(
  EXPORTING
    it_gp               = <ls_curr_line>-gp
    it_anordnungen      = <ls_data>-werte-anordnungen
  CHANGING
    ct_ao               = <ls_curr_line>-ao_reference
    ct_return           = return_tab[]
    cv_success          = success                 " Erfolgskennzeichen
).

    "Stundung
    "Es gibt eine eigene Aktion für Stundung
