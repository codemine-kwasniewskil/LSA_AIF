FUNCTION /thkr/check_fica_utk.
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     REFERENCE(ACTIVITY) TYPE  FM_AUTHACT
*"     REFERENCE(FM_AREA) TYPE  FIKRS OPTIONAL
*"     REFERENCE(FM_FINCODE_AUTHGRP) TYPE  FM_AUTHGRF OPTIONAL
*"     REFERENCE(FM_FMFCTR_AUTHGRP) TYPE  FM_AUTHGRC OPTIONAL
*"     REFERENCE(FM_FIPEX) TYPE  FM_FIPEX OPTIONAL
*"     REFERENCE(FM_MEASURE_AUTHGRP) TYPE  FM_AUTHGR_MEASURE OPTIONAL
*"     REFERENCE(FM_FAREA_AUTHGRP) TYPE  FM_AUTHGR_FAREA OPTIONAL
*"     REFERENCE(IV_USER) TYPE  SY-UNAME DEFAULT SY-UNAME
*"  EXPORTING
*"     REFERENCE(EX_SUBRC) TYPE  N
*"----------------------------------------------------------------------

  DATA ls_check TYPE c LENGTH 6.

  IF activity IS INITIAL.
    RETURN.
  ENDIF.

  IF fm_area IS NOT INITIAL.
    ls_check+0(1) = 'X'.
  ENDIF.
  IF fm_fincode_authgrp IS NOT INITIAL.
    ls_check+1(1) = 'X'.
  ENDIF.
  IF fm_fmfctr_authgrp IS NOT INITIAL.
    ls_check+2(1) = 'X'.
  ENDIF.
  IF fm_fipex IS NOT INITIAL.
    ls_check+3(1) = 'X'.
  ENDIF.
  IF fm_measure_authgrp IS NOT INITIAL.
    ls_check+4(1) = 'X'.
  ENDIF.
  IF fm_farea_authgrp IS NOT INITIAL.
    ls_check+5(1) = 'X'.
  ENDIF.

  CASE ls_check.
    WHEN 'XXXXXX'.
      AUTHORITY-CHECK OBJECT  'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
      " jeweils ein Wert fehlt
    WHEN 'XXXXX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'XXXX X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XXX XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XX XXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'X XXXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' XXXXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
      " jeweils zwei werte fehlen
    WHEN ' XXXX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' XXX X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' XX XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' X XXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '  XXXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'X XXX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X XX X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'X X XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'X  XXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XX  XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XX X X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XX XX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'XXX  X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XXX X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'XXXX  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
      " wenn drei Werte fehlen
    WHEN ' X XX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN ' XX X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN ' XXX  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '  XX X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' X X X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' XX  X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '  X XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' X  XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '   XXX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '  XXX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X  XX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X X X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X XX  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X  X X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'X X  X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'X   XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XX   X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN 'XX  X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'XX X  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'XXX   '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
      "--- wenn jeweils vier werte Fehlen
    WHEN 'XX    '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X X   '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X  X  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X   X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN 'X    X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN ' XX   '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN ' X X  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN ' X  X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN ' X   X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '  XX  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '  X X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '  X  X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '   XX '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '   X X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
    WHEN '    XX'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
      "--- wenn jeweils fünf Werte Fehlen
    WHEN 'X     '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   FIELD fm_area
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN ' X    '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' FIELD fm_fincode_authgrp
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '  X   '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' FIELD fm_fmfctr_authgrp
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '   X  '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   FIELD fm_fipex
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '    X '.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' FIELD fm_measure_authgrp
        ID 'FM_GRP_FAR' DUMMY.
    WHEN '     X'.
      AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
        ID 'FM_AUTHACT' FIELD activity
        ID 'FM_FIKRS'   DUMMY
        ID 'FM_AUTHGRF' DUMMY
        ID 'FM_AUTHGRC' DUMMY
        ID 'FM_FIPEX'   DUMMY
        ID 'FM_AUTHGRM' DUMMY
        ID 'FM_GRP_FAR' FIELD fm_farea_authgrp.
      "--- wenn kein Wert übergeben wird
    WHEN OTHERS.
      IF activity IS NOT INITIAL.

        AUTHORITY-CHECK OBJECT 'Z_FICA_UTK' FOR USER iv_user
       ID 'FM_AUTHACT' FIELD activity
       ID 'FM_FIKRS'   DUMMY
       ID 'FM_AUTHGRF' DUMMY
       ID 'FM_AUTHGRC' DUMMY
       ID 'FM_FIPEX'   DUMMY
       ID 'FM_AUTHGRM' DUMMY
       ID 'FM_GRP_FAR' DUMMY.

      ELSE.
        ex_subrc = 0.
        RETURN.
      ENDIF.
  ENDCASE.

  ex_subrc = Sy-subrc.



ENDFUNCTION.
