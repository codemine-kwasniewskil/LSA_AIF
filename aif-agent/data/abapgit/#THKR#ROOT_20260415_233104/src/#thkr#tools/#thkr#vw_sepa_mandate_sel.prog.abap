...
*& Datum:        14.01.2026                                            *
*&                                                                     *
*& l. Änderung: 27.01.2026                                             *
*&                                                                     *
*&---------------------------------------------------------------------*
REPORT /thkr/vw_sepa_mandate_sel.

************************************************************************
* Globale Variablen                                                    *
************************************************************************
DATA: gv_mguid      TYPE sepa_mguid.
...
SELECTION-SCREEN BEGIN OF BLOCK a1 WITH FRAME TITLE a1_titel.
  ...
  SELECTION-SCREEN BEGIN OF BLOCK bl2 WITH FRAME TITLE TEXT-f02.
    PARAMETERS: p_aus   TYPE xchar AS CHECKBOX,
                p_mguid TYPE sepa_mguid.
...
START-OF-SELECTION.
  IF p_aus IS INITIAL.
    ...
    SELECT * FROM sepa_mandate INTO gs_sepa
      ...
      IF gt_loe_sepa_m IS INITIAL.
        APPEND gs_sepa TO gt_sepa.
      ELSE.
*      READ TABLE gt_loe_sepa_m TRANSPORTING NO FIELDS WITH KEY mndid = gs_sepa-mndid.
        READ TABLE gt_loe_sepa_m TRANSPORTING NO FIELDS WITH KEY mguid = gs_sepa-mguid.
        IF 0 NE sy-subrc.
          APPEND gs_sepa TO gt_sepa.
        ENDIF.
      ENDIF.
    ENDSELECT.
    IF NOT gt_sepa IS INITIAL.
      LOOP AT gt_sepa ASSIGNING <gfs_sepa>.
        CLEAR gs_loe_sepa_m.
        MOVE-CORRESPONDING <gfs_sepa> TO gs_loe_sepa_m.
        MOVE: sy-uname TO gs_loe_sepa_m-cpnam,
              sy-datum TO gs_loe_sepa_m-cpdat,
              sy-uzeit TO gs_loe_sepa_m-cptim.
        INSERT /THKR/loe_sepa_m FROM gs_loe_sepa_m.
      ENDLOOP.
    ENDIF.
    COMMIT WORK.
    SELECT * FROM /THKR/loe_sepa_m INTO TABLE gt_loe_sepa_m
...
*       i_structure_name   = '/THKR/LOE_SEPA_M'
      ...
      EXCEPTIONS
        ...
        OTHERS             = 2.
  ELSE.
    SELECT SINGLE mguid FROM /thkr/loe_sepa_m INTO gv_mguid
                  WHERE mguid EQ p_mguid.
    IF 0 EQ sy-subrc.
      DELETE FROM /thkr/loe_sepa_m WHERE mguid EQ gv_mguid.
      IF 0 EQ sy-subrc.
        MESSAGE 'Ausnahme aus THKR-Tabelle entfernt!'   TYPE 'I'.
      ELSE.
        MESSAGE 'Ausnahme: Fehler beim DELETE!'         TYPE 'E' DISPLAY LIKE 'I'.
      ENDIF.
    ELSE.
      MESSAGE 'Ausnahme: Daten - Selektion FEHLERHAFT!' TYPE 'E' DISPLAY LIKE 'I'.
    ENDIF.
  ENDIF.
